...
@Component({
    ...
    styleUrls: ['./map-radar.scss'],
    imports: [CardModule, ButtonModule, SliderModule, CommonModule, NgIf, FormsModule, Dialog],
    standalone: true
})
export class MapRadar implements OnInit, OnDestroy, AfterViewInit {
    ...
    openMoreInfoRadar(): void {
        window.open('https://gpm.nasa.gov/missions/GPM#:~:text=Concepto%20de%20misi%C3%B3n%20de%20GPM,-La%20misi%C3%B3n%20de&text=Mediante%20la%20mejora%20de%20las,M%C3%A1s%20informaci%C3%B3n%20sobre%20el%20lanzamiento.');
      }
    ...
    private updatePointerValue(lngLat: { lng: number, lat: number } | null): void {
        ...
        // Para radar, mostramos la reflectividad en dBZ
        this.pointerData = `${value.value.toFixed(1)} dBZ`;
    }
}
