...
@Component({
    selector: 'app-map-viento',
    templateUrl: './map-viento.html',
    styleUrls: ['./map-viento.scss'],
    imports: [CardModule, ButtonModule, SliderModule, CommonModule, FormsModule, Dialog],
    standalone: true
})
export class MapViento implements OnInit, OnDestroy, AfterViewInit {
    @ViewChild('mapContainer', { static: true }) mapContainer!: ElementRef;

    private map!: Map;
    private weatherLayer!: WindLayer;
    ...
    openMoreInfoWind(): void {
        window.open('https://www.earthdata.nasa.gov/topics/atmosphere/wind-speed/data-access-tools', '_blank');
      }
    ...
    private initializeMap(): void {
        ...
        this.weatherLayer = new WindLayer({
            opacity: 0.8,
        });
    ...
    private updatePointerValue(lngLat: { lng: number, lat: number } | null): void {
        ...
        this.pointerData = `${value.speedMetersPerSecond.toFixed(1)} m/s`;
    }
}
